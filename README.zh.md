...
│   ├── config_manager.c/h  # NVS 配置持久化（版本 6）
...
1. **NVS 闪存初始化** - 配置存储准备
2. **配置管理器初始化** - 加载设备配置（版本 6）
3. **状态 LED 初始化** - GPIO33 系统状态指示
4. **SPIFFS 挂载** - Web 界面文件系统准备
5. **SD SPI 总线释放** - 为摄像头初始化释放 GPIO14
6. **WiFi 子系统初始化** - 网络接口和事件循环
7. **WiFi 状态回调注册** - 网络状态变化监听
8. **健康监控初始化** - 系统指标收集
9. **WiFi 模式选择** - STA 已配置则连接，否则启动 AP
10. **MJPEG 流传输器初始化** - 实时视频流准备（STA 模式延迟到连接后）
11. **Web 服务器启动** - HTTP 服务和 API 端点（STA 模式延迟到连接后）
12. **SD 卡初始化** - 在摄像头之后初始化，处理 GPIO14 共享

### 重要说明

- **摄像头初始化被延迟到 WiFi STA 连接后**，这是已知的 ESP32 DMA 冻结问题的工作around（esp32-camera#620）
- 在 STA 模式下，MJPEG、时间同步、Web 服务器和运动检测等功能将在 WiFi 连接完成后异步启动
- 项目中 **没有 NAS 上传和 WebDAV 客户端功能**，相关文件已移除
- 项目 **没有任何单元测试或集成测试**，所有验证都是运行时错误检查

## 配置

所有设置存储在 NVS 中（配置版本 6），可通过 Web 界面或 REST API 访问。
...
4. **无 NAS 功能**：版本更新中移除了 NAS 上传和 WebDAV 客户端相关功能
