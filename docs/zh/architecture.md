...
- **模块化**：22 个独立模块，具有定义的接口
- **事件驱动**：具有状态机的异步操作

## 模块架构

固件由 22 个相互连接的模块组成，每个模块具有特定职责：
...
### 13. 帧广播器（`frame_broadcaster.c/h`）
**职责**：帧缓冲分发给多个消费者
- **消费者**：MJPEG 流传输器、视频录制器、运动检测
- **缓冲**：带引用计数的共享帧缓冲

### 14. NAS 上传器（`nas_uploader.c/h`）
**职责**：通过 WebDAV/HTTP 上传到 NAS
- **协议**：WebDAV 或 HTTP POST
- **重试**：可配置重试和指数退避
- **队列**：带优先级管理的上传队列

### 15. WebDAV 客户端（`webdav_client.c/h`）
**职责**：WebDAV 协议实现
- **操作**：PUT、MKCOL、PROPFIND
- **认证**：Basic/Digest 认证

### 16. ONVIF 发现（`onvif_discovery.c/h`）
**职责**：ONVIF WS-Discovery 服务
- **协议**：基于 UDP 多播的 WS-Discovery
- **兼容性**：Synology、Milestone NVR

### 17. ONVIF 服务（`onvif_service.c/h`）
**职责**：ONVIF SOAP 服务处理
- **服务**：设备、媒体、PTZ（模拟）

### 18. WebSocket 服务器（`ws_server.c/h`）
**职责**：通过 WebSocket 实时推送事件
- **事件**：运动、录制、系统状态
- **协议**：基于 HTTP 升级的 WebSocket
